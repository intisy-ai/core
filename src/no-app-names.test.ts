...
// Only file-scoped exemptions, each naming a contract that is not this library's to change:
//   src/testing.ts  the published contract-test kit. Its TempHomes {claude, opencode} shape is
//                   what every plugin's contract test consumes, so renaming it is a cross-repo
//                   API change and belongs to the sub-project that rebases those plugins.
const ALLOWED = new Set(["src/testing.ts"]);
